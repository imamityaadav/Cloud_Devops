...
resource "aws_iam_policy" "custom_assume_role_policy" {
  name        = "custom-assume-role-policy"
  description = "Custom policy to allow assuming jenkins-role"
  policy      = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_iam_role_policy_attachment" "custom_policy_attachment" {
  ...
  policy_arn = aws_iam_policy.custom_assume_role_policy.arn
}
...
resource "aws_key_pair" "jumpbox_key" {
  key_name   = "${var.customer_name}-${var.environment}-jumpbox-keypair"
  public_key = tls_private_key.jumpbox_private_key.public_key_openssh
}

resource "aws_ssm_parameter" "jumpbox_private_key_ssm" {
  name        = "/${var.environment}/ec2-keypairs/${var.customer_name}-${var.environment}-jumpbox-keypair"
  type        = "SecureString"
  value       = tls_private_key.jumpbox_private_key.private_key_pem
  description = "Private key for ${var.customer_name}-${var.environment}-jumpbox-keypair"
  overwrite   = true
...
locals {
  constant_jumpbox_instance_tags = {
    Name        = "${var.customer_name}-${var.environment}-eks-jumpbox"
    ManagedBy   = "Terraform"
  }
}
...
resource "aws_instance" "master" {
  ...
  root_block_device {
    ...
    tags = merge(
    local.constant_jumpbox_instance_tags,
    var.jumpbox_tags
  )
